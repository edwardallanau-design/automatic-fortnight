...
- **Multi-branch table setup + dashboard filtering** — **Built as Story 20** (2026-07-10/11, Plans 1-3 in `BUILD_STATUS.md`): a new `Branch` entity, branch-scoped `OrderingPoint`/auth/menu-sold-out state, admin branch CRUD + selectors, and a branch tab strip on the staff dashboard with `GET /api/orders` now genuinely branch-scoped server-side (previously a real cross-branch data leak — staff at one branch saw every branch's orders). This entry is left as-shipped history of the original idea: for a single restaurant business operating multiple physical branches/locations (distinct from the multi-*tenant* item above, which is about serving separate client businesses), each branch needed its own set of tables and QR codes, and the staff dashboard needed to filter incoming orders by branch so staff at one location aren't seeing another location's orders. Raised 2026-07-10 after Story 19's UX revision smoke test.
  - **Follow-up backlog: dedicated UI/UX pass for the branch tab strip and branch-name tag.** Story 20's three plans shipped functionally correct — server-side security fix, working admin tab strip, client-side filtering with no extra network calls, all verified via smoke test — but without a dedicated visual-design pass, unlike Story 15/16's later "Real UI/UX" treatment of the admin pages. Known gaps to revisit: (1) the branch tab strip reuses the same `.order-rail__tab` styling as the existing Pending/Confirmed tabs with only a small margin tweak to distinguish the "outer" (branch) facet from the "inner" (status) one — easy to visually conflate at a glance; (2) the branch-name tag on `OrderCard` (`.order-card__branch-tag`) duplicates the customer-name tag's CSS verbatim rather than getting its own distinct visual treatment; (3) the tab strip currently shows for any admin with `branches.length > 0`, including a single-branch deployment (a redundant "All / <one branch>" strip that filters nothing) — worth deciding whether to gate on `branches.length > 1` instead. Raised 2026-07-11 during Plan 3's final whole-branch review and manual smoke test.
