...
- **Multi-branch table setup + dashboard filtering** — for a single restaurant business operating multiple physical branches/locations (distinct from the multi-*tenant* item above, which is about serving separate client businesses): each branch would have its own set of tables and QR codes, and the staff dashboard would need to filter incoming orders by branch — either a filter control or a separate tab per branch — so staff at one location aren't seeing another location's orders. Raised 2026-07-10 after Story 19's UX revision smoke test. Needs a decision on the underlying model (a `branch`/`location` field on `Table`? a new `Branch` entity?) and whether this is single-tenant-multi-branch (one owner, several physical sites, same DB) or actually overlaps with the multi-tenant item — not designed yet, just captured.
