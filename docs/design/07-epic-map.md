...
- **Staff-assisted ordering** — let staff place an order on a customer's behalf (e.g. for a customer who doesn't want to use their own phone), reusing the existing table-ordering flow rather than a separate staff-only order form. Raised 2026-07-08; the existing `/order/test` dev table picker may be a useful starting point for how staff would select a table.
