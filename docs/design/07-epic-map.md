...
- **Real UI/UX for Login, Table Setup, and Menu Management** — these three pages (`app/login/page.tsx`, `app/admin/tables/page.tsx`, `app/admin/menu-items/page.tsx`) are still functional-only: unstyled `<h1>`/`<ul>`/plain `<input>` markup with no `className`s, no design-token usage, no responsive/mobile-first treatment — unlike the rest of the app (customer ordering flow, staff dashboard, and the staff/admin toolbar), which all follow the established café-ticket visual language (Fraunces/Inter/JetBrains Mono, the `--espresso`/`--crema`/`--copper` palette). Raised 2026-07-09 during the staff/admin order-edits branch, after the global `StaffBar` polish made the contrast with these three pages more visible.
