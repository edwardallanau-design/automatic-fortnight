...
**ADR-006: Transactional order journal (`OrderEvent`), not event-driven infrastructure**
- **Context.** The Unconfirm feature (2026-07-25) requires an audit trail of order mutations, and future integrations (payment gateway, analytics, kitchen display) were pushing toward "adopt event-driven architecture" as a prerequisite. Vercel serverless cannot natively host long-lived brokers or consumers, and the service layer already owns every order mutation (no route touches Prisma directly).
- **Decision.** An append-only `OrderEvent` table written **in the same database transaction** as each order mutation in the service layer (`INV-17`). All order mutations are journaled, not just privileged ones. Attribution is **role-level only** (ADR-003's shared credentials): action + actor role + timestamp. No broker, no queue, no async consumers, no event sourcing.
- **Alternatives rejected.** Full EDA (broker + consumers) — new always-on infrastructure with zero current consumers; the need was research-sourced, not friction-sourced. Event sourcing — rewrites the persistence model for no current requirement. Journaling only privileged actions — same write cost as journaling everything, strictly less history. Per-person attribution (named PINs or user accounts) — deferred; role + timestamp cross-referenced with the shift roster suffices at the pilot's scale.
- **Assumption that makes this right.** All order mutations flow through the service layer, and at most ~2 people hold the admin credential, so role + timestamp is de-facto attributable.
- **What would invalidate it.** (a) A real integration needing push semantics — a verified payment webhook, a kitchen display — at which point `OrderEvent` becomes the transactional **outbox** read by a dispatcher, an additive step, not a redesign. (b) The client demanding per-person attribution (or >2 admin-credential holders) — then ADR-003 is revisited (named supervisor PINs, or per-user accounts) and journal rows gain an actor identity.
