...
**Known limitation — and it is still open.** The gate lives in the repo, and Vercel evaluates it against *the branch being built*, so a branch is protected only once `vercel.json` has reached it. As of 2026-08-04 that means `dev` alone: `main` and `client/kapeadri` still carry the ungated config, so a push to either still builds into the client's project and runs migrate+seed against its live database. Closing it needs `dev → main` then `main → client/kapeadri` (`B-27`, deferred deliberately — promoting rebuilds a live restaurant's instance). This is the general cost of config-as-code for deploy gates: reviewable and self-propagating, but with a protection lag equal to the promotion cadence.
