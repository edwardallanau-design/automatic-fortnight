...
| ISSUE-10 | `/order/[id]` shows "Order #N confirmed" as the ticket heading while the order is still `Pending` (editable) — the status note directly below it correctly says "Remove items or cancel while your order is still pending," so the two lines contradict each other on the same screen | User report while verifying Story 10a's dashboard tabs/tile work | Major | Open — bug is in `OrderTicket.tsx`'s `heading` prop, reused verbatim from the actual-Confirmed branch in `page.tsx` without updating the copy for the Pending state. Fix: drop "confirmed" from the Pending-state heading (`Order #{N}`, matching the Cancelled branch's plain `Order #{N}` heading). |
